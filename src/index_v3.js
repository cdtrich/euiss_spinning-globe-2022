...
const createChart = async () => {
  ...
  // transitions
  let dataLand = await d3.json(
      "https://unpkg.com/world-atlas@1/world/110m.json"
    ),
    dataDots = d3.json("src/cities.geojson");

  function ready(error, world, dots, cities) {
    if (error) throw error;

    var globe = { type: "Sphere" },
      land = topojson.feature(dataLand, world.objects.land),
      dots = topojson.feature(dataDots, dots.objects.foo).features,
      type = dots.type,
      i = -1,
      n = dots.length;
